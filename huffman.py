...
class HuffmanCoding:
	...
	def make_frequency_dict(self):
		...
		combo = np.stack((values, counts), axis=-1)
		frequency = dict(combo)
		return frequency
	...
	def get_encoded_text(self):
		encoded_text = ""
		for ix,iy in np.ndindex(np.shape(self.img)):
			encoded_text += self.codes[self.img[ix,iy]]
		return encoded_text
